...
> [1] “三方”指：用户访问的网站（内容网站）与 Cookie 所在域（AdFlux）不在同一域下。通过 SameSite=None 属性和 requestStorageAccess API，可以在部分浏览器中继续跨域访问三方 Cookie，详见 `tracker.js` 中注释。经过测试，在默认配置下，Chrome、Firefox、Edge 浏览器可以成功读取三方 Cookie（但可以在设置中禁用），而 Safari 在默认配置下无法读取（需要手动启用）。（浏览器更新后可能失效）
>
